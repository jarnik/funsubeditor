...
public class SubEditor implements PluginInterface {
	...
	public function SubEditor():void {};
	...
	public function clone(tgt:*,adc:Boolean=false):MovieClip {
		var cls:Class = Object(tgt).constructor;
		trace("cloning " + tgt.name + " " + getQualifiedClassName( tgt ) );
	...
	private function buildList(clr:Boolean):void {
		...
		if ( clr ) {
			lineList = [];
			lineList.push({description:"desc", duration:123});
		}
		...
		proportion = lineList.length*buttonheight/hei;
		if (proportion > 1.01) {
		...
		for(var i:Number=0; i<lineList.length; i++) {
			if(clr) {
				...
				var btn:MovieClip = clone(clip.list.button,true);
				var stc:Stacker = new Stacker(btn);
				btn.y = i*buttonheight;
				btn.buttonMode = true;
				btn.mouseChildren =false;
				btn.name = i.toString()+"butt";
				trace( "clone "+ btn['description'] );
				buttons.push({c:btn,s:stc});
				setContents(i);
				trace("adding button ");
			}
			trace("rearranging "+wid);
			buttons[i].s.rearrange(wid);
			trace("x "+buttons[i].c.visible);
		}
	};
	...
	private function itemHandler(evt:ControllerEvent):void {
		...
		if(light) {
			for (var itm:String in lineList[idx]) {
		...
		if(!isNaN(active)) {
			if(front) {
				for (var act:String in lineList[active]) {
	...
	private function setContents(idx:Number):void {
		...
		/*
		trace();
		trace("text? " + buttons[0].c);
		for ( var j:String in buttons[0].c ) {
			trace("prop: "+j+" "+buttons[0].c[j]);
		}*/
		
		//buttons[0].c['description'].text = Strings.digits(123);
		//return;
		...
		for (var itm:String in lineList[idx]) {
			if(!buttons[idx].c[itm]) {
				...
				if(lineList[idx]['image']) {
					...
					ldr.load(new URLRequest(lineList[idx]['image']));
				}
			} else if(itm == 'duration') {
				if(lineList[idx][itm] > 0) {
					buttons[idx].c[itm].text = Strings.digits(lineList[idx][itm]);
					if(front) { 
						buttons[idx].c[itm].textColor = front.color;
					}
				}
				...
				if(itm == 'description') { 
					buttons[idx].c[itm].htmlText = lineList[idx][itm];
				} else { 
					buttons[idx].c[itm].text = lineList[idx][itm];
				}
				if(front) {
					buttons[idx].c[itm].textColor = front.color;
				}
		...
		if(buttons[idx].c['image'] && !lineList[idx]['image']) {
